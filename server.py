...
# ✅ Use a requests Session with keep-alive disabled
...
def get_movie_details(movie_id):
    try:
        ...
        details_res = session.get(details_url, timeout=15)
        details_res.raise_for_status()
        movie_details = details_res.json()

        credits_res = session.get(credits_url, timeout=15)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()  # ✅ creates table if not already there
